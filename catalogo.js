...
function mapearProduto(id, data) {
    ...
    // 🆕 PREÇO PROMOCIONAL — campo opcional no painel. Regras:
    //   - "precoOriginal" é sempre o preço de tabela (o que já existia).
    //   - "precoPromocional" é o valor bruto cadastrado no campo novo do
    //     painel (pode ser null se vazio, ou inválido se >= preço normal).
    //   - "emPromocao" só fica true quando o promocional existe, é > 0 e é
    //     MENOR que o preço normal — assim um valor digitado errado (maior
    //     ou igual ao normal) simplesmente não aplica, em vez de "aumentar"
    //     o preço do produto sem querer.
    //   - "preco" continua sendo o campo que o resto do site usa pra
    //     carrinho/checkout/"a partir de" — ele já sai calculado como o
    //     preço EFETIVO (promocional quando válido, normal quando não), pra
    //     nenhum outro lugar do site.html precisar saber que promoção existe.
    const precoOriginal = Number(data.preco) || 0;
    const precoPromocional = (data.precoPromocional !== undefined && data.precoPromocional !== null && data.precoPromocional !== "")
        ? Number(data.precoPromocional)
        : null;
    const emPromocao = precoPromocional !== null && precoPromocional > 0 && precoPromocional < precoOriginal;

    return {
        id,
        cat: data.cat || "Copos",
        nome: data.nome || "",
        preco: emPromocao ? precoPromocional : precoOriginal,
        precoOriginal,
        precoPromocional,
        emPromocao,
        desc: data.desc || "",
...
export async function salvarProduto(db, dados, id = null) {
    const payload = {
        ...
        // 🆕 Preço promocional — grava null quando o campo fica vazio (sem
        // promoção), em vez de 0 (que apareceria como "produto de graça" em
        // qualquer checagem futura que esqueça de checar "emPromocao").
        precoPromocional: (dados.precoPromocional !== undefined && dados.precoPromocional !== null && String(dados.precoPromocional).trim() !== "")
